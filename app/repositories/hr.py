...
class DepartmentRepository(BaseRepository[Department, DepartmentCreate, DepartmentUpdate]):
    ...
    def get_instructors_count(self, department_id: int) -> int:
        ...
        return self.db.query(Instructor).filter(
...
class ContractRepository(BaseRepository[Contract, ContractCreate, ContractUpdate]):
    ...
    def get_instructors_count(self, contract_id: int) -> int:
        ...
        return self.db.query(Instructor).filter(
            Instructor.contract_id == contract_id
        ).count()
...
class InstructorRepository(BaseRepository[Instructor, InstructorCreate, InstructorUpdate]):
    ...
    def get_with_relations(self, instructor_id: int) -> Optional[Instructor]:
        ...
        return self.db.query(Instructor).options(
            joinedload(Instructor.contract),
            joinedload(Instructor.department)
        ).filter(Instructor.instructor_id == instructor_id).first()
    ...
    def get_workload_summary(self, instructor_id: int) -> dict:
        ...
        return {
            ...
            "contract_limit": instructor.contract.hour_limit if instructor.contract else None,
            "available_hours": (
