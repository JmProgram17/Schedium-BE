...
class ScheduleRepository(BaseRepository[Schedule, ScheduleCreate, ScheduleUpdate]):
    ...
    def get_groups_count(self, schedule_id: int) -> int:
        ...
        return self.db.query(StudentGroup).filter(
            StudentGroup.schedule_id == schedule_id
        ).count()
...
class TimeBlockRepository(BaseRepository[TimeBlock, TimeBlockCreate, TimeBlockUpdate]):
    ...
    def get_day_time_blocks_count(self, time_block_id: int) -> int:
        ...
        return self.db.query(DayTimeBlock).filter(
...
class DayTimeBlockRepository(BaseRepository[DayTimeBlock, DayTimeBlockCreate, None]):
    ...
    def get_by_day_and_block(self, day_id: int, time_block_id: int) -> Optional[DayTimeBlock]:
    ...
    def get_with_relations(self, day_time_block_id: int) -> Optional[DayTimeBlock]:
        ...
        return self.db.query(DayTimeBlock).options(
            joinedload(DayTimeBlock.day),
            joinedload(DayTimeBlock.time_block)
        ).filter(DayTimeBlock.day_time_block_id == day_time_block_id).first()
    ...
    def get_by_day_name_and_time(
        ...
        return self.db.query(DayTimeBlock).join(
            Day
        ).join(
            TimeBlock
        ).filter(
...
class ClassScheduleRepository(BaseRepository[ClassSchedule, ClassScheduleCreate, ClassScheduleUpdate]):
    ...
    def get_with_relations(self, class_schedule_id: int) -> Optional[ClassSchedule]:
        ...
        return self.db.query(ClassSchedule).options(
            joinedload(ClassSchedule.quarter),
            joinedload(ClassSchedule.day_time_block).joinedload(DayTimeBlock.day),
            joinedload(ClassSchedule.day_time_block).joinedload(DayTimeBlock.time_block),
        ...
        ).filter(ClassSchedule.class_schedule_id == class_schedule_id).first()
