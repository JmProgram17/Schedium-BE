...
from app.schemas.academic import (
    Level, LevelCreate, LevelUpdate,
    Chain, ChainCreate, ChainUpdate,
    Nomenclature, NomenclatureCreate, NomenclatureUpdate,
    Program, ProgramCreate, ProgramUpdate,
    StudentGroup, StudentGroupCreate, StudentGroupUpdate, StudentGroupDisable
)
...
async def get_nomenclatures(
    ...
    return SuccessResponse(
        data=nomenclatures,
        message="Nomenclatures retrieved successfully"
    )
...
@router.get("/nomenclatures/{nomenclature_id}", response_model=SuccessResponse[Nomenclature])
async def get_nomenclature(
    ...
    return SuccessResponse(
        data=nomenclature,
        message="Nomenclature retrieved successfully"
    )


@router.put("/nomenclatures/{nomenclature_id}", response_model=UpdatedResponse[Nomenclature])
...
async def get_programs(
    params: PaginationParams = Depends(get_pagination_params),
    search: Optional[str] = Query(None, description="Search in program name or nomenclature"),
...
async def get_student_groups(
    params: PaginationParams = Depends(get_pagination_params),
    search: Optional[str] = Query(None, description="Search by group number or program"),
    ...
    groups = service.get_student_groups(
        params, search, program_id, schedule_id, active,
...
@router.patch("/groups/{group_id}/disable", response_model=UpdatedResponse[StudentGroup])
async def disable_student_group(
    ...
    return UpdatedResponse(
        data=group,
        message="Student group disabled successfully"
    )
