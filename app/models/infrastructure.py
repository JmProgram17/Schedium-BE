...
class Classroom(Base, TimeStampMixin):
    ...
    campus_id = Column(Integer, ForeignKey("campus.campus_id", ondelete="RESTRICT"), nullable=False)
...
class DepartmentClassroom(Base, TimeStampMixin):
    ...
    __table_args__ = {"comment": "Many-to-many relationship between departments and classrooms"}
    
    department_id = Column(Integer, ForeignKey("department.department_id", ondelete="CASCADE"), primary_key=True)
    classroom_id = Column(Integer, ForeignKey("classroom.classroom_id", ondelete="CASCADE"), primary_key=True)
    priority = Column(Integer, default=0, comment="Booking priority level")
    is_primary = Column(Boolean, default=False, comment="Is primary classroom for department")
