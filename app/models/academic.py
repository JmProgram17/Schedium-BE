...
class Nomenclature(Base, TimeStampMixin):
    ...
    __table_args__ = {"comment": "Standardized abbreviations or codes for academic programs"}
    
    nomenclature_id = Column(Integer, primary_key=True, autoincrement=True)
    code = Column(String(20), nullable=False, unique=True, index=True, comment="Abbreviation or short code")
...
class Program(Base, TimeStampMixin):
    ...
    nomenclature_id = Column(Integer, ForeignKey("nomenclature.nomenclature_id", ondelete="SET NULL"))
...
class StudentGroup(Base, TimeStampMixin):
    ...
    schedule_id = Column(Integer, ForeignKey("schedule.schedule_id", ondelete="RESTRICT"))
    active = Column(Boolean, default=True, nullable=False)
