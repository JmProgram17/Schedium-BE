...
class Schedule(Base, TimeStampMixin):
    ...
    __table_args__ = {"comment": "Daily schedules for classes (morning, afternoon, evening, etc.)"}
...
class TimeBlock(Base, TimeStampMixin):
    ...
    def __repr__(self):
        return f"<TimeBlock(id={self.time_block_id}, {self.start_time}-{self.end_time})>"
...
class DayTimeBlock(Base, TimeStampMixin):
    ...
    __table_args__ = {"comment": "Association between days and time blocks for scheduling"}
    
    day_time_block_id = Column(Integer, primary_key=True, autoincrement=True)
    time_block_id = Column(Integer, ForeignKey("time_block.time_block_id", ondelete="CASCADE"), nullable=False)
    day_id = Column(Integer, ForeignKey("day.day_id", ondelete="CASCADE"), nullable=False)
...
class ClassSchedule(Base, TimeStampMixin):
    ...
    __table_args__ = {"comment": "Class schedules linking instructors, groups, classrooms, and time slots"}
    
    class_schedule_id = Column(Integer, primary_key=True, autoincrement=True)
    subject = Column(String(255), nullable=False)
    quarter_id = Column(Integer, ForeignKey("quarter.quarter_id", ondelete="CASCADE"), nullable=False)
    day_time_block_id = Column(Integer, ForeignKey("day_time_block.day_time_block_id", ondelete="RESTRICT"), nullable=False)
    group_id = Column(Integer, ForeignKey("student_group.group_id", ondelete="CASCADE"), nullable=False)
    instructor_id = Column(Integer, ForeignKey("instructor.instructor_id", ondelete="RESTRICT"), nullable=False)
    classroom_id = Column(Integer, ForeignKey("classroom.classroom_id", ondelete="RESTRICT"), nullable=False)
    ...
    __table_args__ = (
        UniqueConstraint('day_time_block_id', 'instructor_id', 'quarter_id', 
                        name='uq_schedule_conflict_instructor'),
        UniqueConstraint('day_time_block_id', 'classroom_id', 'quarter_id', 
                        name='uq_schedule_conflict_classroom'),
        UniqueConstraint('day_time_block_id', 'group_id', 'quarter_id', 
